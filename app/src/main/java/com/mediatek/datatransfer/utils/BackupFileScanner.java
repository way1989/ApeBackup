    ...
    private class ScanThread extends Thread {
        ...
        public void run() {
            if (!SDCardUtils.isSdCardAvailable(mContext)) {
                MyLogger.logW(CLASS_TAG, "ScanThread.run(): No SD card found");
                return;
            }
            isCanceled = false;
        ...
        private String makeOneBackupHistory(List<File> result) throws IOException {
            ...
            FileUtils.combineFiles(Arrays.asList(contactDestFile.listFiles()), contactDestFile
                    .getAbsolutePath()
                    + File.separator + ModulePath.NAME_CONTACT);
            for (File file : contactDestFile.listFiles()) {
                if (file.getName().equals(ModulePath.NAME_CONTACT)) {
                    continue;
                }
